...
#         self.voxel_files = self.voxel_files  # Only use a small subset for quick training
...
#         file = self.voxel_files[idx]
#         label = self.label_dict[file.stem]
#         return file.name, label  # or file.stem if you want to drop `.npy`
...
# ------------ Training Loop ------------
def train(model, dataset, train_loader, val_loader, device, output_dir, epochs=2000, lr=1e-4):
    ...
    # Efficient class counting using precomputed labels
    train_labels = [dataset.labels[i] for i in train_loader.dataset.indices]
    class_counts = Counter(train_labels)
    weights = torch.tensor([1 / (class_counts.get(i, 1e-5)) for i in range(8)], device=device)
...
if __name__ == '__main__':
    ...
    output_dir = Path('/home/fei/Documents/Dataset/icra25_align_human_robot/labeled/robot/cam_104122061850/pick/weights_004/run1')
    ...
    train_losses, val_losses = train(model, dataset, train_loader, val_loader, device, output_dir)
